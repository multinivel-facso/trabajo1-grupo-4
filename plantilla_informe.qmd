...
-   El promedio de felicidad de un país atenúa el efecto del nacionalismo en la opinión frente a la migración.

-   El promedio de percepción de seguridad de un país atenúa el efecto del nacionalismo en la opinión frente a la migración
...
Los métodos utilizados fueron la estimación de distintos modelos bajo una lógica de carácter multinivel, obteniendo un total de 6 modelos utilizando el programa Rstudio y la librería lmer. Se estimaron un modelo nulo para ver la variabilidad según país que se comentará más adelante, modelos con variables de nivel 1, nivel 2, nivel 1 + nivel 2, uno con pendiente aleatoria en nacionalismo, y dos modelos de interacción entre niveles, uno para ver la interacción entre nacionalismo y felicidad promedio por país, y el otro para ver la interacción entre nacionalismo y seguridad promedio. Las fórmulas de los modelos fueron las siguientes:
...
------------------------------------------------------------------------
*Modelo con interacción entre niveles seguridad y nacionalismo*

$$
\begin{aligned}
\text{op\_mig}_{ij} &= \gamma_{00} + \gamma_{01} \text{meanschooling} + \gamma_{02} \text{happiness\_promedio\_pais} + \gamma_{03} \text{seguridad\_promedio\_pais} \\
&+ \gamma_{04} \text{female} + \gamma_{05} \text{pos\_pol} + \gamma_{06} \text{nacionalismo} + \gamma_{07} \text{personal\_income} \\
&+ \gamma_{08} \text{seguridad\_promedio\_pais} \times \text{nacionalismo} + u_{0j} + u_{1j} \text{nacionalismo} + r_{ij}
\end{aligned}
$$
...
results_6 = lmer(op_mig ~ 1 + meanschooling_gmc + happiness_gmc + seguridad_gmc + female + pos_pol_cmc + nacionalismo_cmc + personal_income_cmc + seguridad_gmc*nacionalismo_cmc + (1 | pais), data = data)

modelo_6 <- screenreg(results_6)#Modelo 5, interacción entre niveles

ICC_6 <- reghelper::ICC(results_6)

```

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
tab_modelos <- tab_model(results_0, results_1, results_2, results_3, results_4, results_5, results_6,
  show.ci = FALSE, 
  show.se = TRUE,
  dv.labels = c("Nulo","Nivel 1", "Nivel 2", "Nivel 1 y 2", "Pendiente aleatoria nacionalismo", "Interacción entre niveles Felicidad", "Interacción entre niveles Seguridad"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
graf_interacción_seguridad <- plot_model(results_6,
           type = "int",
           terms = c("nacionalismo_cmc", "seguridad_gmc")) +
  labs(
    title  = "Interacción: Nacionalismo individual × Promedio de seguridad percibida por país",
    x      = "Nacionalismo (centrado)",
    y      = "Opinión sobre migración",
    colour = "Promedio de seguridad percibida por país"
  ) +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3")) +
  theme_minimal(base_size = 12) +
  theme(
...
Como se muestra en la Figura @fig-4
