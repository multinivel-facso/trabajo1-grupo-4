...
save(data, file="~/Documents/GitHub/trabajo1-grupo-4/output/data3.rdata")
...
```{r descriptivos nivel1, echo=FALSE, results='asis'}
library(stargazer)
...
```{r descriptivos nivel2, echo=FALSE, results='asis'}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
results_2 = lmer(perc_mig ~ 1 + happiness_gmc + meanschooling_gmc + (1 | pais), data = data)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
results_3 = lmer(perc_mig ~ 1 + happiness_gmc + meanschooling_gmc + female + pos_pol_cmc + nacionalismo_cmc + personal_income_cmc + (1 | pais), data = data)
...
reg_agg=lm(perc_mig ~ + happiness_gmc + meanschooling_gmc, data=agg_mlm)

reg_pend_aleatoria=lmer(perc_mig ~ 1 + happiness_gmc + meanschooling_gmc + female + pos_pol_cmc + nacionalismo_cmc + personal_income_cmc +( 1 + nacionalismo | pais), data = data)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
tab_modelos <- tab_model(results_0, results_1, results_2, results_3, reg_pend_aleatoria, 
  show.ci = FALSE, 
  show.se = TRUE,
...
```{r}
reg_4= lmer(perc_mig ~ democ + meanschooling + female + pos_pol + nacionalismo + personal_income + (1 | pais), data = data)

inf_reg_4 <- influence(reg_4, "pais")

cook <- cooks.distance(inf_reg_4, sort = TRUE)

corte <- 4/46 # cut point

inluyentes <- names(cook)[!is.na(cook) & cook > corte] 

cat("Países influyentes (cook > ", round(corte, 3), "):\n") 

plot(inf_reg_4, which="cook",
     cutoff=corte, sort=TRUE,
     xlab="Cooks Distance",
     ylab="Pais")

sigtest(inf_reg_4, test=-1.96)$meanschooling[1:46,]
```
...
data=filter(data,!pais %in% c(124, 608, 554,360, 858, 792))
table(data$perc_mig)

results_2b = lmer(perc_mig ~ 1 + meanschooling + hdi + unemploytotal + democ + (1 | pais), data = data)
results_2b

hdi = lmer(perc_mig ~ 1 + migrationrate + (1 | pais), data = data)
table(data$Q121)
tab_modelosb <- tab_model(hdi, 
  show.ci = FALSE, 
  show.se = TRUE,
  dv.labels = c( "Agregado"))
tab_modelosb
M <- cor(data, use = "complete.obs")
M
sjPlot::plot_scatter(data = agg_mlm, 
                     x = mean_income_pais,
                     y = perc_mig,
                     fit.line = "lm",
                     label.var = agg_mlm$pais)

scater <- ggplot(agg_mlm, aes(x = migrationrate, y = perc_mig, label = pais)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  geom_text(vjust = -0.5, size = 3) +
  theme_minimal() +
  labs(x = "Ingreso promedio por país",
       y = "Porcentaje de migrantes",
       title = "Relación entre ingreso promedio y migración")
...
agg_mlm <- agg_mlm %>%
  group_by(pais) %>%
  mutate(mean_income_pais = mean(personal_income, na.rm = TRUE)) %>%
  ungroup()
```
