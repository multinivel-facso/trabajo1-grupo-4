...
nivel2 <- data %>%
  select(pais, democ, hdi, meanschooling) %>%
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
results_2 = lmer(perc_mig ~ 1 + happines_promedio_pais + meanschooling + (1 | pais), data = data)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
results_3 = lmer(perc_mig ~ 1 + happines_promedio_pais + meanschooling + female + pos_pol + nacionalismo + personal_income + (1 | pais), data = data)
...
reg_agg=lm(perc_mig ~ + migrationrate + meanschooling, data=agg_mlm)

reg_pend_aleatoria=lmer(perc_mig ~ 1 + meanschooling + female + pos_pol + nacionalismo + personal_income +( 1 + nacionalismo | pais), data = data)
```
